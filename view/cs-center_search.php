    ...
    <table class="table table-hover table-striped text-center" style="border: lpx solid">
      ...
      <?php 
          ...
          $page_start = ($page - 1) * $list;
                    ...
                    /* 게시글 정보 가져오기  */
          $sql = "SELECT * FROM question_list where $catagory like '%$search_con%' ORDER BY number DESC LIMIT $page_start, $list";
          $result2 = $connect->query($sql);
          ?>
      <tbody>
        <?php
              while($rows = mysqli_fetch_array($result2)){ //DB에 저장된 데이터 수 (열 기준)
                  if($total_record%2==0){
            ?> <tr class="even">
          <?php   }
                else{
